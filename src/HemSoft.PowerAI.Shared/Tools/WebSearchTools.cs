...
public static class WebSearchTools
{
    ...
    [Description("Search the web for current info. Returns summaries with titles, URLs, and content.")]
    public static Task<string> WebSearchAsync(string query) =>
        WebSearchCoreAsync(query, maxResults: DefaultMaxResults);
    ...
    /// <returns>Formatted search results or error message.</returns>
    [Description("Search the web for current info. Returns summaries with titles, URLs, and content.")]
    public static Task<string> WebSearchAsync(string query, int maxResults) =>
        WebSearchCoreAsync(query, maxResults);
    ...
    private static async Task<string> WebSearchCoreAsync(string query, int maxResults)
    {
        ...
        return await ExecuteSearchAsync(query, apiKey, maxResults, new Uri(apiUrl)).ConfigureAwait(false);
    }

    private static async Task<string> ExecuteSearchAsync(string query, string apiKey, int maxResults, Uri apiUrl)
    {
        try
        {
    ...
    {
        _ = sb.Append("- **").Append(result.Title).AppendLine("**")
              .Append("  URL: ").AppendLine(result.Url?.ToString() ?? string.Empty);
